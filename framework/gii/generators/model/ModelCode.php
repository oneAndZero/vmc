...
class ModelCode extends CCodeModel
{
	...
	public function validateTableName($attribute,$params)
	{
		...
		if($invalidTables!=array())
			$this->addError('tableName', 'Model class cannot take a reserved PHP keyword! Table name: '.implode(', ',(array)$invalidTables).".");
		if($invalidColumns!=array())
			$this->addError('tableName', 'Column names that does not follow PHP variable naming convention: '.implode(', ',(array)$invalidColumns).".");
	}
	...
	public function generateRules($table)
	{
		...
		if($required!==array())
			$rules[]="array('".implode(', ',(array)$required)."', 'required')";
		if($integers!==array())
			$rules[]="array('".implode(', ',(array)$integers)."', 'numerical', 'integerOnly'=>true)";
		if($numerical!==array())
			$rules[]="array('".implode(', ',(array)$numerical)."', 'numerical')";
		if($length!==array())
		{
			foreach($length as $len=>$cols)
				$rules[]="array('".implode(', ',(array)$cols)."', 'length', 'max'=>$len)";
		}
		if($safe!==array())
			$rules[]="array('".implode(', ',(array)$safe)."', 'safe')";
