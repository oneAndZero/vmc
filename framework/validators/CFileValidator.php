...
class CFileValidator extends CValidator
{
	 ...
	 * This can be either an array or a string consisting of file extensions names
	 ...
	 * In order to use this property fileinfo PECL extensions should be installed.
	 * @since 1.1.11
	 */
	public $mimeTypes;
	/**
	...
	public $tooSmall;
	/**
	 * @var string the error message used when the uploaded file has an extensions name
	 ...
	 * fileinfo PECL extensions should be installed.
	 * @since 1.1.11
	 */
	...
	protected function validateFile($object, $attribute, $file)
	{
		...
		else if($error==UPLOAD_ERR_INI_SIZE || $error==UPLOAD_ERR_FORM_SIZE || $this->maxSize!==null && $file->getSize()>$this->maxSize)
		{
			...
			throw new CException(Yii::t('yii','File upload was stopped by extensions.'));
		...
		if($this->types!==null)
		{
			...
			if(!in_array(strtolower($file->getExtensionName()),$types))
			{
				...
				$this->addError($object,$attribute,$message,array('{file}'=>$file->getName(), '{extensions}'=>implode(', ',(array)$types)));
			}
		}
		...
		if($this->mimeTypes!==null)
		{
			if(function_exists('finfo_open'))
			{
				...
				throw new CException(Yii::t('yii','In order to use MIME-type validation provided by CFileValidator fileinfo PECL extensions should be installed.'));
			...
			if($mimeType===false || !in_array(strtolower($mimeType),$mimeTypes))
			{
				...
				$this->addError($object,$attribute,$message,array('{file}'=>$file->getName(), '{mimeTypes}'=>implode(', ',(array)$mimeTypes)));
			}
		}
	}
