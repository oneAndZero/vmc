...
class CMssqlCommandBuilder extends CDbCommandBuilder
{
	...
	protected function joinOrdering($orders, $newPrefix)
	{
		if(count($orders)>0)
		{
			...
			$orderBy = 'ORDER BY '.implode(', ',(array)$str);
	...
	protected function checkCriteria($table, $criteria)
	{
		...
		{
			$criteria->order=is_array($table->primaryKey)?implode(',',(array)$table->primaryKey):$table->primaryKey;
		}
		return $criteria;
	}
	...
	protected function createCompositeInCondition($table,$values,$prefix)
	{
		...
		foreach($values as $value)
		{
			...
			$vs[]='('.implode(' AND ',(array)$c).')';
		}
		return '('.implode(' OR ',(array)$vs).')';
	}
}
