...
class CDbCommandBuilder extends CComponent
{
	...
	public function createInsertCommand($table,$data)
	{
		...
		$sql="INSERT INTO {$table->rawName} (".implode(', ',(array)$fields).') VALUES ('.implode(', ',(array)$placeholders).')';
		$command=$this->_connection->createCommand($sql);
	...
	public function createUpdateCommand($table,$data,$criteria)
	{
		...
		$sql="UPDATE {$table->rawName} SET ".implode(', ',(array)$fields);
		$sql=$this->applyJoin($sql,$criteria->join);
		$sql=$this->applyCondition($sql,$criteria->condition);
		$sql=$this->applyOrder($sql,$criteria->order);
		$sql=$this->applyLimit($sql,$criteria->limit,$criteria->offset);
	...
	public function createColumnCriteria($table,$columns,$condition='',$params=array(),$prefix=null)
	{
		...
		{
			if($criteria->condition!='')
				$criteria->condition=implode(' AND ',(array)$conditions).' AND ('.$criteria->condition.')';
			else
				$criteria->condition=implode(' AND ',(array)$conditions);
		}
		return $criteria;
	}
	...
	public function createSearchCondition($table,$columns,$keywords,$prefix=null,$caseSensitive=true)
	{
		...
		foreach($columns as $name)
		{
			...
			$conditions[]=implode(' AND ',(array)$condition);
		}
		return '('.implode(' OR ',(array)$conditions).')';
	}
	...
	public function createInCondition($table,$columnName,$values,$prefix=null)
	{
		...
		if(is_string($columnName)) // simple key
		{
			...
			if($n===1)
				...
				return $prefix.$column->rawName.' IN ('.implode(', ',(array)$values).')';
		}
		else if(is_array($columnName)) // composite key: $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
		{
			...
			if(count($values)===1)
			{
				...
				return implode(' AND ',(array)$entries);
			}
	...
	protected function createCompositeInCondition($table,$values,$prefix)
	{
		...
		$vs=array();
		foreach($values as $value)
			$vs[]='('.implode(', ',(array)$value).')';
		return '('.implode(', ',(array)$keyNames).') IN ('.implode(', ',(array)$vs).')';
	}
