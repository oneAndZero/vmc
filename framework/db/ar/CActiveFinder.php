...
class CJoinElement
{
	...
	private function applyLazyCondition($query,$record)
	{
		...
		if($this->relation instanceof CManyManyRelation)
		{
			...
			if($parentCondition!==array() && $childCondition!==array())
			{
				...
				$join.='('.implode(') AND (',(array)$parentCondition).') AND ('.implode(') AND (',(array)$childCondition).')';
	...
	public function count($criteria=null)
	{
		...
		$select=is_array($criteria->select) ? implode(',',(array)$criteria->select) : $criteria->select;
	...
	private function joinOneMany($fke,$fks,$pke,$parent)
	{
		...
		return $this->relation->joinType . ' ' . $this->getTableNameWithAlias() . ' ON (' . implode(') AND (',(array)$joins).')';
	}
	...
	private function joinManyMany($joinTable,$fks,$parent)
	{
		...
		if($parentCondition!==array() && $childCondition!==array())
		{
			...
			$join.=' ON ('.implode(') AND (',(array)$parentCondition).')';
			$join.=' '.$this->relation->joinType.' '.$this->getTableNameWithAlias();
			$join.=' ON ('.implode(') AND (',(array)$childCondition).')';
			if(!empty($this->relation->on))
				$join.=' AND ('.$this->relation->on.')';
			return $join;
		}
...
class CJoinQuery
{
	...
	{
		$sql=($this->distinct ? 'SELECT DISTINCT ':'SELECT ') . implode(', ',(array)$this->selects);
		$sql.=' FROM ' . implode(' ',(array)$this->joins);
		...
		if($conditions!==array())
			$sql.=' WHERE (' . implode(') AND (',(array)$conditions).')';
		...
		foreach($this->groups as $group)
			if($group!=='')
				$groups[]=$group;
		if($groups!==array())
			$sql.=' GROUP BY ' . implode(', ',(array)$groups);
		...
		foreach($this->havings as $having)
			if($having!=='')
				$havings[]=$having;
		if($havings!==array())
			$sql.=' HAVING (' . implode(') AND (',(array)$havings).')';
		...
		foreach($this->orders as $order)
			if($order!=='')
				$orders[]=$order;
		if($orders!==array())
			$sql.=' ORDER BY ' . implode(', ',(array)$orders);
...
class CStatElement
{
	...
	private function queryOneMany()
	{
		...
		else  // composite FK
		{
			...
			$sql='SELECT '.implode(', ',(array)$cols).", {$relation->select} AS $s FROM {$table->rawName} ".$tableAlias.$join
				.$where.'('.$builder->createInCondition($table,$fks,$keys,$tableAlias.'.').')'
				.' GROUP BY '.implode(', ',(array)array_keys($cols)).$group
				.$having.$order;
			$command=$builder->getDbConnection()->createCommand($sql);
			if(is_array($relation->params))
				$builder->bindValues($command,$relation->params);
	...
	private function queryManyMany($joinTableName,$keys)
	{
		...
		$sql='SELECT '.$this->relation->select.' AS '.$schema->quoteColumnName('s').', '.implode(', ',(array)$cols)
			.' FROM '.$table->rawName.' '.$tableAlias.' INNER JOIN '.$joinTable->rawName
			.' ON ('.implode(') AND (',(array)$joinCondition).')'.$join
			.$where
			.' GROUP BY '.implode(', ',(array)array_keys($cols)).$group
